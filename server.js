	...
	if(req.session.username)
	{
		...
		fs.readFile(userFile, "utf8", function(error, data)
		{
			...
			else
			{
				...
				if(sessionuser)
				{
					resp.sendFile(__dirname + '/views/deposit.html');
				}
				else
				{
	...
	if(req.session.username)
	{
		...
		fs.readFile(userFile, "utf8", function(error, data)
		{
			...
			else
			{
				...
				if(sessionuser)
				{
					resp.sendFile(__dirname + '/views/withdraw.html');
				}
				else
				{
					resp.send(getMessageHtml("Unable to fetch user file!!!"));
	...
	if(req.session.username)
	{
		...
		fs.readFile(userFile, "utf8", function(error, data)
		{
			...
			else
			{
				...
				if(sessionuser)
				{
					resp.sendFile(__dirname + '/views/transfer.html');
				}
				else
				{
					resp.send(getMessageHtml("Unable to fetch user file!!!"));
...
app.get('/viewnewuser', function(req, resp){
	resp.sendFile(__dirname + '/views/newuser.html');
});

app.get('/viewnewaccount', function(req, resp){
	if(req.session.username)
	{
		resp.sendFile(__dirname + '/views/newaccount.html');
	}
	else
	{
		resp.redirect('/');		
